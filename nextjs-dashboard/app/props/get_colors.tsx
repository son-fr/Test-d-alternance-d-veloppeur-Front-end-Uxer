...
export type ColorsValues =  'unset' |
                            /*Neutral*/
                            '#fafafa' | '#f5f5f5' | '#e5e5e5' | '#d4d4d4' | '#a3a3a3' | '#737373' | '#525252' | '#404040' | '#262626'  | '#171717' | '#0a0a0a' |
                            '#ffffff0d' | '#ffffff1a' | '#ffffff33' | '#ffffff4d' | '#ffffff66' | '#ffffff80' | '#ffffff99' | '#ffffffb3' | '#ffffffcc' | '#ffffffe6' | '#ffffff' |
                            '#0000000d' | '#0000001a' | '#00000033' | '#0000004d' | '#00000066' |  '#00000080' | '#00000099' | '#000000b3' | '#000000cc' | '#000000e6' | '#000000' |
                            /*Brand*/
                            '#F1F9F1' | '#DBF0DB' | '#B9E1B8' | '#8ECD8C' | '#5DB86B' | '#2F9F42' | '#007031' | '#005a27' | '#00441d' | '#002e14' | '#001f0c' |
                            '#fff5f4' | '#ffe4e0' | '#ffc3bc' | '#ff9a90' | '#f96c5d' | '#e9422e' | '#ca220e' | '#a1180b' | '#791009' | '#510a06' | '#370604' |
                            '#f9f5fa' | '#f2e6f6' | '#e3cbea' | '#d0a8db' | '#b881c8' | '#9957b1' | '#7a367f' | '#602964' | '#471c49' | '#311132' | '#1f0a1f' |
                            '#f5efe5' | '#ede2cf' | '#e3d3b5' | '#d9c39b' | '#ceb17e' | '#c4a264' | '#bc954e' | '#a37f3e' | '#816531' | '#5c4823' | '#372b15' |
                            /*Semantic*/
                            '#fef3f2' | '#fee4e2' | '#fecdca' | '#fda29b' | '#f97066' | '#f04438' | '#d92d20' | '#b42318' | '#912018' | '#7a271a' | '#55160c' |
                            '#fffaeb' | '#fef0c7' | '#fedf89' | '#fec84b' | '#fdb022' | '#f79009' | '#dc6803' | '#b54708' | '#93370d' | '#7a2e0e' | '#4e1d09' |
                            '#ecfdf3' | '#dcfae6' | '#abefc6' | '#75e0a7' | '#47cd89' | '#17b26a' | '#079455' | '#067647' | '#085d3a' | '#074d31' | '#053321' |
                            '#eff8ff' | '#d1e9ff' | '#b2ddff' | '#84caff' | '#53b1fd' | '#2e90fa' | '#1570ef' | '#175cd3' | '#1849a9' | '#194185' | '#102c60' |
                            '#ecfdff' | '#cff9fe' | '#a5f0fb' | '#67e3f9' | '#22ccee' | '#06aed4' | '#088ab3' | '#0e7090' | '#155b76' | '#164c63' | '#0b3445' |
                            '#eef4ff' | '#e0eaff' | '#c7d7fe' | '#a4bcfd' | '#8098f9' | '#6172f3' | '#444ce7' | '#3538cd' | '#2d31a6' | '#2d3282' | '#1f235b' |
                            '#fdf2fa' | '#fce7f6' | '#fcceee' | '#faa7df' | '#f670c7' | '#ee46bc' | '#dd2590' | '#c11574' | '#9e165f' | '#851651' | '#4c0d2f' |
                            '#fff1f3' | '#ffe4e8' | '#fecdd6' | '#fea3b4' | '#fd6f8e' | '#f63d68' | '#e31b54' | '#c01048' | '#a11043' | '#89123e' | '#510b24' |
                            '#fefbe8' | '#fef7c3' | '#feee95' | '#fde272' | '#fac515' | '#eaaa08' | '#ca8504' | '#a15c07' | '#854a0e' | '#713b12' | '#4d2009';

export type Colors_state = {
    default?: Colors | ColorsValues;
    hover?: Colors | ColorsValues;
    focus?: Colors | ColorsValues;
    disabled?: Colors | ColorsValues;
}

export default function Get_Colors(colors_state: Colors_state = {
            default: 'Neutral/Grey/50',
            hover: 'Neutral/Grey/50',
            focus: 'Neutral/Grey/50',
            disabled: 'Neutral/Grey/50',
        }) {
    const Colors_values: Colors_state = {
            default : Get_Color_value(colors_state.default),
            hover: Get_Color_value(colors_state.hover),
            focus: Get_Color_value(colors_state.focus),
            disabled: Get_Color_value(colors_state.disabled),
        };
    /*if (Colors_values.hover == 'unset')
        Colors_values.hover = Colors_values.default;
    if (Colors_values.focus == 'unset')
        Colors_values.focus = Colors_values.default;*/
...
function Get_Color_value(color: Colors | ColorsValues = 'Neutral/Grey/50') {
    ...
    let Color_value : ColorsValues = '#fafafa';

    for (let i = 0; i != list_Colors[0].length; i++) {
        if (list_Colors[0][i] === color) {
            Color_value = list_Colors[1][i];
            break;
        }
    }
    return(
